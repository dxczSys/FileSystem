...
Vue.component('paging-more', {
    template: '<button class="btn btn-link btn-block btn-sm" onclick="offset += 1;getPage();"><b>获取更多</b></button><br/><br/>'
});
...
function setResources(resources, tabId) {
    ...
    if (resources.length < 1) {
        ...
        $.each(resources, function (i, resource) {
            ...
            contentHtml += `<div class="content-box rounded" data-id="${resource.id}">`
            
            contentHtml += `<div class="file-row">`
            contentHtml += `<img src="${resource.avatar ? resource.avatar : '/assets/img/default-user.jpg'}" class="rounded acter-byme"/>`
            contentHtml	+= `<div class="file-mess file-names" style="flex-grow: 1;max-width:300px;"><a data-toggle="tooltip" class="visit-url" href="${resource.visitUrl}" target="_blank" data-description="${resource.fileName}" title="${resource.fileName}">${resource.fileName}</a></div>`
            "#uploaded-content" === tabId && (contentHtml += `<div class="edit-delete-group"><span onclick="removeFile()">删除</span></div>`) 
            contentHtml += `</div>`
            
           	contentHtml += `<div class="file-row" style="line-height:30px;">`
           	contentHtml += `<div class="file-mess" style="width:150px;margin-left:42px;">上传者：${resource.username}</div>`
            contentHtml += `<div class="file-mess" style="width:160px;">${new Date(date).format("yyyy-MM-dd hh:mm:ss")}</div>`
            contentHtml += `<div class="file-mess" style="width:90px;">${formatSize(resource.size)}</div>`
            contentHtml += `<div class="file-mess" style="width:150px;">分类：${resource.categoryName}</div>`
            contentHtml += `<div class="file-mess" style="width:300px;flex-grow: 1;" data-toggle="tooltip" data-description="${resource.description}" title="${resource.description}">描述：${resource.description}</div>`
            contentHtml += `<div class="file-mess" style="width:100px;">下载次数：${resource.downloadTimes}</div>`
            contentHtml += `</div>`
            
            contentHtml += `</div>`
//            	<span onclick="editFile()">编辑</span>
//            contentHtml += "<div class='row content-box rounded' data-id='" + resource.id + "'><div class='col-12 col-sm-12'><div class='row'>" +
//                (isMobile() ? "" : "") +
//                "<div class='col-sm-11 col-12'><img src='" + (resource.avatar ? resource.avatar : "/assets/img/default-user.jpg") + "' class='rounded avatar'/><h4><a data-toggle='tooltip' class='visit-url' href='" + resource.visitUrl + "' target='_blank' data-description='" + resource.description + "' title='" + resource.description + "'>" + resource.fileName + "</a>" +
...
//                "</h4><p>上传者：<b>" + resource.username + "</b>" + (isDownloaded ? "下载" : "上传") + "时间：<b>" + new Date(date).format("yyyy-MM-dd hh:mm:ss") + "</b>&emsp;文件大小：<b>" + formatSize(resource.size) + "</b>&emsp;分类：<b class='file-category'>" + resource.categoryName + "</b>" +
